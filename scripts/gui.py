...
class samm_visualizer():
    ...
    def generate_mask(self):
        ...
        if self.zero_shot is not None and self.zero_shot == 2:
            point_label = [_ + 2 if _ > -1 else _ for _ in point_label]
        if len(point) == 0:
            point = None
            point_label = None
        else:
        ...
        if self.zero_shot is not None and self.zero_shot == 1:
            prompt_class = np.array([600])[np.newaxis,...]
        else:
            prompt_class = np.array([1])[np.newaxis,...]
