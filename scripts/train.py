...
from .utils.workflow_utils import generate_prompt_pairs, get_next_points, generate_prompt_pairs_val, sample_points_patch_val,none_cat, MERGE_LIST, USE_SV_GT_LIST
...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    metric_dim = parser.get_parsed_content("output_classes") - 1 # only affect dice calculation
    random_seed = parser.get_parsed_content("random_seed")
    save_last = parser.get_parsed_content("save_last", default=False)  # save the last checkpoint
    save_all = parser.get_parsed_content("save_all", default=False) # save all val-checkpoint
    if world_size == 1 or dist.get_rank() == 0:
    ...
    weighted_sampling = parser.get_parsed_content("weighted_sampling") # perform dataset weighted sample using dataset_weights.yaml
    skip_iter_prob = parser.get_parsed_content("skip_iter_prob") # prob. to skip iterative point sampling, 1 for auto-training
    iter_num = parser.get_parsed_content("iter_num") # total iter number in point branch training
    freeze_epoch = parser.get_parsed_content("freeze_epoch", default=-1) # freeze the whole branch epoch
    freeze_head = parser.get_parsed_content("freeze_head", default='auto') # freeze which branch, "auto" or "point". We freeze point for auto-training.
    ...
    json_dir = parser.get_parsed_content("json_dir", default='./data/jsons') # path to json datalists
    ...
    if train_transforms is not None:
        ...
        train_transforms.append(RelabelD(label_key, label_mappings=label_mappings, dtype=torch.int32))
    ...
    for _round in range(num_rounds) if world_size > 1 and dist.get_rank() != 0 else progress_bar:
        model.train()
        epoch_loss = 0
        ...
        for batch_data in train_loader:
            ...
            for _k in range(inputs_l.shape[0] // num_patches_per_iter):
                ...
                if labels_p is not None:
                    label_prompt_pseudo, point_pseudo, point_label_pseudo, prompt_class_pseudo = generate_prompt_pairs(labels_p,
