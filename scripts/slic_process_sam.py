...
class InferClass:
    ...
    def infer(self, image_file='example/s1238.nii.gz', rank=0, output_dir='./supervoxel_sam/', data_root_dir=None, n_segments=400):
        ...
        for file in image_file:
            if data_root_dir is not None:
                savefolder = os.path.join(output_dir, file.replace(data_root_dir, '').split('/')[0], file.replace(data_root_dir, '').split('/')[1].replace('.nii.gz', ''))
            else:
                savefolder = os.path.join(output_dir, file.split('/')[-1].replace('.nii.gz', ''))
            ...
            try:
                ...
                batch_data = transforms.ScaleIntensityRange(a_max=1000,a_min=-1000,b_max=255,b_min=0,clip=True)(batch_data)
                ...
                for views in permute_pairs:
                    ...
                    for i in tqdm(range(int(np.ceil(data.shape[0] / max_slice)))):
                        ...
                        image = torch.nn.functional.pad(image, (0, pad_w, 0, pad_h, 0, 0))
                        image = monai.transforms.Resize([d, 1024, 1024], mode='bilinear')(image.unsqueeze(0)).squeeze(0)
                        image = torch.stack([image, image, image], -1).permute(0, 3, 1, 2).contiguous()
                        image = (image - pixel_mean) / pixel_std
                        feature = self.model.get_feature_upsampled(image.to(f'cuda:{rank}'))
                        feature = monai.transforms.Resize([h+pad_h, w+pad_w, d], mode='bilinear')(feature.permute(1,2,3,0))[:,:h,:w]
                    ...
                    features = None
                start = time.time()
                outputs = slic(features_.numpy(), channel_axis=0, compactness=0.01, n_segments=n_segments, sigma=3)
                features_ = None
                outputs = torch.from_numpy(outputs).cuda()
                print('slic took', time.time()-start)
                mask = monai.transforms.utils.get_largest_connected_component_mask(orig_data < -800, connectivity=None, num_components=1).cuda()
    ...
    def batch_infer(self, datalist=str, basedir=str, output_dir='./supervoxel_sam/', data_root_dir=None, n_segments=400):
        ...
        infer_files = partition_dataset(
            data=train_files, shuffle=False, num_partitions=world_size, even_divisible=False)[rank]
        self.infer(infer_files, rank=rank, output_dir=output_dir, data_root_dir=data_root_dir, n_segments=n_segments)
