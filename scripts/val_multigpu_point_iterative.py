...
from .utils import generate_prompt_pairs_val, get_next_points_val
...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    if torch.cuda.device_count() > 1:
        dist.init_process_group(backend="nccl", init_method="env://", timeout=timedelta(seconds=10000))
    ...
    with torch.no_grad():
        ...
        for val_data in val_loader:
            ...
            for idx in range(max_iters):
                if idx == 0:
                    _, point, point_label, prompt_class = generate_prompt_pairs_val(val_data["label"].to(device),
                                                                            label_set,
                                                                            max_ppoint=1,  use_center=use_center) 
                    ...
                    if skip_this_iter:
                        if idx < 10:
                            _point, _point_label  = get_next_points_val(val_outputs.transpose(1,0), val_data["label"].to(device), 
                                                                    torch.tensor(label_set).to(device), point, point_label, use_center=False, erosion2d=True)
                            skip_this_iter = torch.all(_point_label[1:, -1] == -1) 
                            if skip_this_iter:
                ...
                with autocast(enabled=amp):
                    ...
                    val_outputs = sliding_window_inference(
                        ...
                        class_vector=None,
                        prompt_class=torch.ones(len(label_set), 1).to(device) * 600,# will not be used when val_point_sampler is not None
                        labels=None,
                        label_set=None,
                        use_cfp=True,
                        brush_radius=None,
                ...
                val_pred = post_transform({'image': val_data['image'][0], 'pred':val_outputs[0]})['pred']
        ...
        if torch.cuda.device_count() > 1:
            dist.barrier()
            global_combined_tensor = [torch.zeros_like(metric) for _ in range(world_size)]
            dist.all_gather(tensor_list=global_combined_tensor, tensor=metric)
            metric = torch.vstack(global_combined_tensor)
            dist.barrier()
            global_combined_tensor = [torch.zeros_like(point_num) for _ in range(world_size)]
