...
from .utils import generate_prompt_pairs_val, get_next_points_val, sample_points_patch_val
# from .monai_utils import compute_robust_hausdorff
from .monai_trans_utils import VistaPostTransform
...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    if save_metric:
        output_dirs = os.path.join(os.path.dirname(parser.get_parsed_content("log_output_file")), dataset_name)
            ...
            with autocast(enabled=amp):
                ...
                if val_auto:
                    try:
                        ...
                        skipped = True
                        logger.debug(f"{_index} / {len(val_loader)} / {val_filename}: skipped due to OOM")
                        continue
                else:
                    for _device_in, _device_out in zip([device, device, "cpu"], [device, "cpu", "cpu"]):
                        try:
                            with autocast(enabled=amp):
                                ...
                                val_outputs = sliding_window_inference(
                                    ...
                                    use_cfp=True,
                                    brush_radius=None,
                                    val_point_sampler=partial(sample_points_patch_val, mapped_label_set=mapped_label_set, max_ppoint=1, use_center=True)) 
                                finished = True
                                skipped = False
                        ...
                        if finished:
                            break
                    if skipped:
                        logger.debug(f"{_index} / {len(val_loader)} / {val_filename}: skipped due to OOM with size {val_data['image'].shape}")
                        continue
                ...
                if argmax_first:
                    ...
                    try:
                        ...
                        for i in range(1, len(label_set)):
                            ...
                            if i == 1 and (dataset_name == 'Task07' or dataset_name == 'Task03'):
                                y_pred = torch.logical_and(y_pred > 0.5,  val_outputs[:, [i+1]] < 0.5)
                            value[0, i-1] = compute_dice(y_pred, y=gt, include_background=False)
                        _final_count += 1
                        if save_metric:
