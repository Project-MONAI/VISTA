...
if __package__ in (None, ""):
    ...
    from utils import generate_prompt_pairs, get_next_points, debug_next_point, generate_prompt_pairs_val, debug_pairs, sample_points_patch_val, Point_sampler, none_cat, MERGE_LIST
else:
    from .monai_utils import sliding_window_inference
    from .utils import generate_prompt_pairs, get_next_points, debug_next_point, generate_prompt_pairs_val, debug_pairs, sample_points_patch_val, Point_sampler,none_cat, MERGE_LIST
...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    skip_train = parser.get_parsed_content("skip_train", default=False)
                ...
                for _index, val_data in enumerate(data_loader[val_times]):
                    ...
                    for _device_in, _device_out in zip([device, device, "cpu"], [device, "cpu", "cpu"]):
                        try:
                            ...
                            #     generate_prompt_pairs_val(val_data["label"].to(_device_in), val_orig_set,
                            #                                 max_ppoint=max(point_start_num, 1),
                            #                                 device=_device_in)
                            label_prompt = label_set
                            label_prompt_global = []
                            for i in label_prompt:
                                label_prompt_global.append(label_mapping[i])
                            ...
                            with autocast(enabled=amp):
                                ...
                                val_outputs = sliding_window_inference(
                                    ...
                                    point_coords=point,
                                    point_labels=point_label,
                                    class_vector=torch.tensor(label_prompt_global).to(device).unsqueeze(0),
