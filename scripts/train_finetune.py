        ...
        for batch_data in train_loader:
            ...
            for _k in range(inputs_l.shape[0] // num_patches_per_iter):
                ...
                if label_prompt is None and point is None:
                    logger.debug(f'Iteration skipped due to None prompts')
                ...
                for _index, val_data in enumerate(data_loader[val_times]):
                    ...
                    for _device_in, _device_out in zip([device, device, "cpu"], [device, "cpu", "cpu"]):
                        try:
                            ...
                            promot_class = torch.ones(len(label_set), 1).to(device) # supported class
                    ...
                    if finished:
                        ...
                        for i in range(1, len(val_orig_set)):
                            if val_times == 0:
                                ...
                                gt = val_data["label_gt"].to(val_outputs.device) == val_orig_set[i]
