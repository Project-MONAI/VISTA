...
from .utils import generate_prompt_pairs_val, get_next_points_val, convert_point_label, generate_prompt_pairs_auto_point
...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    if five_fold:
        ...
        train_files, _ = datafold_read(datalist=data_list_file_path, basedir=data_file_base_dir, fold=-1, key='training')
        val_files, _ = datafold_read(datalist=data_list_file_path, basedir=data_file_base_dir, fold=-1, key='validation')
        test_files, _ = datafold_read(datalist=data_list_file_path, basedir=data_file_base_dir, fold=-1, key='testing')
    process_dict = {'training': train_files, 'validation': val_files, 'testing': test_files, 'all': train_files + val_files + test_files}
    ...
    val_loader = DataLoader(val_ds, num_workers=parser.get_parsed_content("num_workers_validation", default=2), batch_size=1, shuffle=False)
    ...
    if torch.cuda.device_count() > 1:
        model.module.load_state_dict(torch.load(ckpt, map_location=device), strict=False)
    else:
    ...
    with torch.no_grad():
            ...
            for idx in range(max_iters):
                if idx == 0:
                    ...
                    val_outputs = sliding_window_inference(
                        ...
                        point_coords=None,
                        point_labels=None,
                        class_vector=torch.tensor(mapped_label_set).to(device).unsqueeze(0),
                        prompt_class=None,
                        labels=None,
                        label_set=None,
                        use_cfp=True,
                    ...
                    val_outputs_auto = val_outputs.clone()
                    ...
                    for i in range(1, len(label_set)):
                        ...
                        value[0, i-1] = compute_dice(y_pred=val_outputs[:, [i]], y=gt, include_background=False)
                    torch.cuda.empty_cache()
                    print(f"{_index} / {len(val_loader)} / 'auto' / {val_filename}: {value}")
                    logger.debug(f"{_index} / {len(val_loader)} / 'auto' / {val_filename}: {value}")
                    value_auto = value.clone()
                    value_max = value_auto
                    max_avg = value_auto.mean()
                    ...
                    # 3 trials
                    # 1. sample from foreground, 2. random sample from pos/neg region. 3. random sample from pos/neg region, repeat.
                    
                    points, point_labels  = generate_prompt_pairs_auto_point(val_outputs_auto.transpose(1,0), val_data["label"].to(device), 
                                                              torch.tensor(label_set).to(device), mapped=False)
                    ...
                    for j in range(points.shape[1]):
                        val_outputs = sliding_window_inference(
                            ...
                            sw_device=device,
                            device=device,
                            point_coords=points[:,[j]], # not None
                            point_labels=point_labels[:,[j]],
                            class_vector=None,
                            prompt_class=torch.ones(len(label_set), 1).to(device),# supported class
                            labels=None,
                            label_set=None,
                            use_cfp=True,
                            brush_radius=None,
                            prev_mask=val_outputs_auto,
                            val_point_sampler=None) # making sure zero-shot         
                        val_outputs = post_pred(val_outputs[0])[None, ...]
                        val_outputs = post_transform({'image': val_data['image'][0], 'pred':val_outputs[0]})['pred'][None, ...]           
                        value = torch.full((1, metric_dim), float('nan')).to(device)
                        for i in range(1, len(label_set)):
                        ...
                        val_outputs = None
                        torch.cuda.empty_cache()
                        logger.debug(f"{_index} / {len(val_loader)} / {j} / {val_filename}: {value}")
                        print(f"{_index} / {len(val_loader)} / ' point {j}' /{val_filename}: {value_max}")
                        value = torch.vstack([value_auto, value]).max(0)[0].unsqueeze(0)
                        if value.mean() > max_avg:
                            value_max = value
                            max_avg = value.mean()
                            print(f"{_index} / {len(val_loader)} / 'new max at {j}' /{val_filename}: {value_max}")
                            logger.debug(f"{_index} / {len(val_loader)} / 'new max at {j}' / {val_filename}: {value_max}")    
                                               
                        # 
                    for _c in range(metric_dim):
                        val0 = torch.nan_to_num(value_max[0, _c], nan=0.0)
                        val1 = 1.0 - torch.isnan(value_max[0, _c]).float()
                        metric[idx, 2 * _c] += val0
                        metric[idx, 2 * _c + 1] += val1
        ...
        for idx in range(max_iters):
            ...
            if torch.cuda.device_count() == 1 or dist.get_rank() == 0:
                ...
                valid = 0
                for _c in range(metric_dim):
                    if metric[idx, 2 * _c + 1] > 0:
                        v = metric[idx,2 * _c] / metric[idx, 2 * _c + 1]
                        avg_metric += v
                        valid += 1
                    else:
                        v = torch.nan
                ...
                print(f"Avg_metric at {idx}: {avg_metric}")
                logger.debug(f"Avg_metric at {idx}: {avg_metric}")
