...
def remove_module_prefix(input_pth, output_pth):
    ...
    for key, value in checkpoint.items():
        if isinstance(value, dict) and "state_dict" in value:
            ...
            new_state_dict = {k.replace("module.", ""): v for k, v in value["state_dict"].items()}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Remove 'module.' prefix from PyTorch weights")
    parser.add_argument("--input", required=True, help="Path to input .pth file")
    parser.add_argument("--output", required=True, help="Path to save the modified .pth file")
