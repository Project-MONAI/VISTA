...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    if torch.cuda.device_count() == 1 or dist.get_rank() == 0:
        ConfigParser.export_config_file(parser.get(), os.path.join(bundle_root,'configs_validate.yaml'), fmt="yaml", default_flow_style=None)
    ...
    with warnings.catch_warnings():
        ...
        if not valid_at_orig_resolution_only:
            for _round in range(1) :
                ...
                if torch.cuda.device_count() == 1 or dist.get_rank() == 0:
                    logger.debug("----------")
                    logger.debug(f"drop label {drop_label_prob}, drop point {drop_point_prob}")
                model.eval()
                ...
                with torch.no_grad():
                    ...
                    if torch.cuda.device_count() == 1 or dist.get_rank() == 0:
                        ...
                        for _c in range(metric_dim):
                            ...
                            print(f"Evaluation metric - class {_c + 1}: {v}")
                            try:
                                writer.add_scalar(
