...
from .utils import generate_prompt_pairs, get_next_points, debug_next_point, generate_prompt_pairs_val
...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    exclude_background = parser.get_parsed_content("exclude_background", default=False)
    ...
    freeze_epoch = parser.get_parsed_content("freeze_epoch") 
    max_prompt = parser.get_parsed_content("max_prompt")
    max_backprompt = parser.get_parsed_content("max_backprompt")
    ...
    if torch.cuda.device_count() == 1 or dist.get_rank() == 0:
        ConfigParser.export_config_file(parser.get(), os.path.join(bundle_root,'configs.yaml'), fmt="yaml", default_flow_style=None)
    ...
    best_metric_epoch = -1
    idx_iter = 0
    metric_dim = output_classes - 1 if (softmax or exclude_background) else output_classes
    val_devices_input = {}
    val_devices_output = {}
        ...
        if not valid_at_orig_resolution_only:
            ...
            for _round in range(num_rounds) if torch.cuda.device_count() > 1 and dist.get_rank() != 0 else progress_bar:
                ...
                for batch_data in train_loader:
                    if step % (len(train_files)//num_images_per_batch) == 0:
                        epoch = _round * num_epochs_per_validation + step//(len(train_files)//num_images_per_batch)
                        lr_scheduler.step()
                        ...
                        if freeze_epoch > epoch:
                            ...
                            try:
                                model.module.set_auto_grad(requires_grad=False)
                            except:
                                model.set_auto_grad(requires_grad=False)
                            logger.debug(f'Automatic branch frozen at epoch {epoch}!')
                        else:
                            drop_label_prob_train = drop_label_prob
                            drop_point_prob_train = drop_point_prob
                            try:
                                model.module.set_auto_grad(requires_grad=True)
                            except:
                                model.set_auto_grad(requires_grad=True)     
                            logger.debug(f'Automatic branch unfrozen at epoch {epoch}!')   
                        if torch.cuda.device_count() == 1 or dist.get_rank() == 0:
                            logger.debug("----------")
                    ...
                    for _k in range(inputs_l.shape[0] // num_patches_per_iter):
                        ...
                        if torch.cuda.device_count() > 1:
                            if dist.get_rank() == 0:
                                skip_iter = (torch.rand(1) < skip_iter_prob).float().to(device)
                            else:
                                skip_iter = torch.empty(1).float().to(device)
                            dist.barrier()
                        ...
                        if skip_iter > 0:
                            ...
                            drop_iter = random.randint(1, max(num_iters-2, 1))

                        label_prompt, point, point_label, prompt_class, point_mask = generate_prompt_pairs(labels, label_set, image_size=patch_size,
                                                                                               max_point=max_point, 
                        ...
                        for click_indx in range(num_iters):
                            ...
                            if not reuse_embedding or click_indx == num_iters - 1: # not use loss accumulate or the end of iterative training.
                            ...
                            outputs = outputs.detach().float().sigmoid()
                            if click_indx != drop_iter - 1 and click_indx != num_iters - 2: # do not sample new for iter drop_iter and last iter
                                point, point_label, point_mask = get_next_points(outputs, labels, prompt_class, 
                                                                     point, point_label, image_size=patch_size, previous_mask=point_mask)
                                
                                
                                # debug_next_point(outputs, point, point_label, point_mask, labels, prompt_class, click_indx)
                    ...
                    for val_data in val_loader:
                        ...
                        for _device_in, _device_out in zip(device_list_input, device_list_output):
                            try:
                                ...
                                label_prompt, point, point_label, prompt_class, point_mask = \
                                    generate_prompt_pairs_val(val_data["label"].to(_device_in), label_set, 
                                                          image_size=patch_size,
                                                          max_point=max(point_start_num, 1),
                                                          device=_device_in)
                                if drop_point_prob > 0.99:
                                    point = None
                                    point_label = None
                        ...
                        if finished:
                            ...
                            value = compute_dice(
                                ...
                                include_background=not softmax and not exclude_background,
                    ...
                    if torch.cuda.device_count() == 1 or dist.get_rank() == 0:
                        ...
                        for _c in range(metric_dim):
                            if metric[2 * _c + 1] > 0:
                                ...
                                v = torch.nan
                            logger.debug(f"Evaluation metric - class {_c + 1}: {v}")
                            try:
                                writer.add_scalar(
                                    f"val_class/acc_{class_names[_c]}", v, epoch
                                )
                            except BaseException:
