...
from .point_head import Point_Mapping_SAM, Point_Mapping_GAP
Point_Mapping = Point_Mapping_SAM
class SwinUNETR_P(nn.Module):
    """
    ...
    def __init__(
        ...
        self.no_class_embeddings = nn.Embedding(1, feature_size)  
        self.point_mapping = Point_Mapping(feature_size)
    ...
    def mapping_out(self, src, class_vector):
        b, c, h, w, d = src.shape
    ...
    def forward(self, input_images,         
        ...
        labels=None,
        label_set=None):
        """ patch_coords is the patch position passed from scripts.monai_utils.sliding_window_inferer, only
        used in validation.
        """
        hidden_states_out = self.swinViT(input_images, self.normalize)
        enc0 = self.encoder1(input_images)
        ...
        if class_vector is not None:
            label_logits = self.mapping_out(out, class_vector)
            if point_coords is not None:
                
                point_logits = self.point_mapping(out, point_coords, point_labels)
                logits = ((1-weight)*(1-mapping_index.to(float)) + weight).view(-1,1,1,1,1) * label_logits 
            ...
            logits[mapping_index] = self.point_mapping(out, point_coords, point_labels)
        return logits
